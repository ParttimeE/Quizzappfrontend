<script lang="ts">
	import { Button, Modal, Span, Table, TableBody, TableBodyCell, TableBodyRow, TableHead, TableHeadCell } from "flowbite-svelte";
	import type { questionObject } from "./types";
	import { Icon } from "svelte-boxicons";

    export let showModal: boolean
    export let quiz: questionObject
				...
				<TableBody>
					{#each quiz.Questions as question, i}
						<TableBodyRow>
							...
							{:else}
								<TableBodyCell
									><Icon name="bx-x-regular" class="w-5 h-5 bg-red-600" /></TableBodyCell
								>
							{/if}
						</TableBodyRow>
					{/each}
